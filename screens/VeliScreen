// ============================================================
// YUMURCAK — VeliScreen.js
// Veli (çocuk hesabı) ana ekranı — yakında gelecek
// ============================================================

import { View, Text, TouchableOpacity, StyleSheet, SafeAreaView } from 'react-native';
import { RENKLER } from '../constants';

export function VeliEkrani({ kullanici, kres, cikisYap }) {
  return (
    <SafeAreaView style={s.con}>
      <View style={s.ic}>
        <Text style={s.emoji}>👨‍👩‍👧</Text>
        <Text style={s.baslik}>Veli Paneli</Text>
        <Text style={s.alt}>{kres?.ad || 'Kreş'}</Text>
        <Text style={s.alt}>Hoş geldin!</Text>
        <Text style={{ color: RENKLER.altMetin, marginTop: 20 }}>
          Yakında burada çocuğunuzun günlük takibi olacak 🚀
        </Text>
