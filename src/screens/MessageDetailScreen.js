...
// Ortak mesaj detay ekranı: veli <-> öğretmen
// Firebase:
...
export default function MessageDetailScreen() {
  ...
  const { conversationId, conversationMeta = {}, title = 'Mesajlar', subtitle = '' } = route.params || {};
              ...
              return (
                <View style={[styles.messageRow, mine ? styles.messageRowMine : styles.messageRowOther]}>
                  <View style={[styles.bubble, mine ? styles.bubbleMine : styles.bubbleOther]}>
                    <Text style={[styles.messageText, mine ? styles.messageTextMine : styles.messageTextOther]}>{item.metin}</Text>
                    <Text style={[styles.timeText, mine ? styles.timeTextMine : styles.timeTextOther]}>{formatTime(item.createdAt)}</Text>
                  </View>
                </View>
              );
            }}
...
const styles = StyleSheet.create({
  ...
  header: { paddingHorizontal: 14, paddingTop: 10, paddingBottom: 10, flexDirection: 'row', alignItems: 'center', backgroundColor: THEME.bg, borderBottomWidth: 1, borderBottomColor: THEME.border },
  ...
  emptyCard: { backgroundColor: THEME.card, borderRadius: 22, padding: 24, alignItems: 'center', borderWidth: 1, borderColor: THEME.border, marginTop: 20 },
  ...
  timeText: { fontSize: 10, alignSelf: 'flex-end', marginTop: 4, fontWeight: '700' },
  timeTextMine: { color: 'rgba(255,255,255,0.72)' },
  timeTextOther: { color: THEME.muted },
  inputBar: { flexDirection: 'row', alignItems: 'flex-end', padding: 10, backgroundColor: THEME.card, borderTopWidth: 1, borderTopColor: THEME.border },
  input: { flex: 1, maxHeight: 110, backgroundColor: THEME.bg, borderRadius: 18, paddingHorizontal: 14, paddingVertical: 10, color: THEME.text, borderWidth: 1, borderColor: THEME.border, fontWeight: '600' },
  sendButton: { marginLeft: 8, backgroundColor: THEME.primary, borderRadius: 16, paddingHorizontal: 14, paddingVertical: 12, minWidth: 74, alignItems: 'center' },
