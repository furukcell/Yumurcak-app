// ============================================================
// YUMURCAK — AdminMonthlyBulletinScreen.js
// FAZ 8: "Aylık Bülten" — yemek listesi/ders programı gibi GÜN BAZLI
// değil, ay başına TEK kayıt olan bir belge türü. Başlıklı bölümlerden
// (örn. "Bu Ay Öne Çıkanlar", "Yaklaşan Etkinlikler") oluşan, kurum
// genelinde yayınlanan bir haber bülteni.
//
...
const NODE_PATH = 'aylikBultenler';
const KAYNAK = 'admin_aylik';

function defaultSections() {
  return [
    { id: generateId(), baslik: 'Bu Ay Öne Çıkanlar', icerik: '' },
    { id: generateId(), baslik: 'Yaklaşan Etkinlikler', icerik: '' },
    { id: generateId(), baslik: 'Duyurular ve Hatırlatmalar', icerik: '' },
  ];
}
...
export default function AdminMonthlyBulletinScreen({ navigation }) {
  ...
  // Ay değişince: o ay için zaten yayınlanmış bülten varsa onu düzenlemeye
  ...
  async function handleCopyPreviousMonth() {
    ...
    try {
      ...
      if (!prevRecord) {
        Alert.alert('Bulunamadı', 'Geçen ay için yayınlanmış bir bülten bulunamadı.');
        return;
      }
      ...
      Alert.alert('Kopyalandı', 'Geçen ayın bülteni kopyalandı. Değişiklikleri yapıp yayınlayabilirsin.');
    } catch (error) {
      console.log(error);
  ...
  function confirmPublish() {
    ...
    Alert.alert(
      'Bülteni Yayınla',
      `${monthLabel} bülteni yayınlansın mı? Aynı ay için eski yayın pasife alınır.`,
      [
    ...
    try {
      await publishSingleRecord({
        ...
        buildRecord: ({ kresId: kId, monthKey: mKey, kaynak, now }) => ({
          ...
          ayKey: mKey,
          tarih: `${mKey}-01`,
          baslik: baslik.trim() || `${monthLabel} Bülteni`,
          bolumler: bolumler
      ...
      console.log(error);
      Alert.alert('Hata', 'Bülten yayınlanamadı.');
    } finally {
      setSaving(false);
    }
  }
  ...
  function confirmUnpublish() {
    ...
    Alert.alert(
      'Yayından Kaldır',
      `${monthLabel} bülteni kaldırılsın mı?`,
      [
        { text: 'Vazgeç', style: 'cancel' },
        ...
        <ScrollView style={styles.screen} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.header}>
            ...
            <View style={styles.headerTextWrap}>
              <Text style={styles.title}>Aylık Bülten</Text>
              <Text style={styles.subtitle}>Kurum geneli, ay bazlı haber bülteni</Text>
            </View>
          </View>
          ...
          <View style={styles.monthCard}>
            ...
            <View style={styles.monthCenter}>
              ...
              <Text style={styles.monthHint}>{loadingDraft ? 'Yükleniyor...' : 'Bülten taslağı'}</Text>
            </View>
            <TouchableOpacity style={styles.monthButton} onPress={() => changeMonth(1)} activeOpacity={0.8}>
              <Text style={styles.monthButtonText}>›</Text>
            </TouchableOpacity>
          ...
          {publishedCount > 0 ? (
            <View style={styles.publishedCard}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.publishedText}>Veliler ve öğretmenler şu an bu bülteni görüyor.</Text>
              </View>
          ...
          <Text style={styles.fieldLabel}>Bülten Başlığı</Text>
          <TextInput
            value={baslik}
            onChangeText={setBaslik}
            placeholder={`${monthLabel} Bülteni`}
          ...
          <TouchableOpacity disabled={saving} style={[styles.saveButton, { opacity: saving ? 0.6 : 1 }]} onPress={confirmPublish} activeOpacity={0.85}>
            <Text style={styles.saveButtonText}>{saving ? 'Yayınlanıyor...' : `${monthLabel} Bültenini Yayınla`}</Text>
          </TouchableOpacity>
