...
export default function ParentSummaryScreen({ navigation }) {
  ...
  const todayMeal = useMemo(() => {
    ...
    const classMeal = active.find((item) => item.sinifId === sinifId);
    return classMeal || active[0] || null;
    ...
    return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.emptyWrap}>
          <EmptyState icon="👧" title="Sisteme kayıtlı çocuk bulunmuyor" desc="Özet ekranı için çocuğun veli hesabına bağlı olması gerekir." />
        </View>
      </SafeAreaView>
    );
  }

  const mood = todayReport?.mood || todayReport?.ruhHali || todayReport?.durum || 'Henüz girilmedi';
  const sleep = todayReport?.uyku?.sure ? `${todayReport.uyku.sure} saat` : (todayReport?.uykuDurumu || 'Henüz girilmedi');
  const note = todayReport?.not || todayReport?.ogretmenNotu || todayReport?.notlar || 'Bugün için öğretmen notu henüz girilmedi.';
  const attendanceStatus = todayAttendance?.durum || (todayReport ? 'geldi' : '');
  const attendanceLabel = ATTENDANCE_LABELS[attendanceStatus] || attendanceStatus || 'Bekleniyor';
  const entryTime = todayAttendance?.girisSaati || todayAttendance?.saat || todayAttendance?.createdAtSaat || '-';
...
const createStyles = (theme) => StyleSheet.create({
  ...
  wideCard: { backgroundColor: theme.card, borderRadius: 21, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: theme.border, flexDirection: 'row', alignItems: 'center', gap: 12 },
